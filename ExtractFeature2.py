...
def checkvalid(arr):
    ...
    y_keypoints = [y for idx, y in enumerate(
        arr_new) if idx % 3 == 1]
    if any(i < 0 for i in arr_new) or any(i > 256 for i in y_keypoints) or any(i > 192 for i in x_keypoints):
        return False
    else:
        return True
...
def extract_keypoints(json_file_path):
    ...
    for item in data:
        ...
        if item['image_id'] not in image_id_list:
            ...
            dis_list.append(d)
            item['keypoints'].insert(0, item['image_id'])
...
def find_files(name, path):
    ...
    return result
# def data_label(file_name, features):
